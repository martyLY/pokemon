...
static const unsigned int MAXHP = 1500;
static const double WSP = 0.4;
static const double AVOID = 0.05;
...
//分为主要属性和次要属性
void HighBlood::initPokemon() {
    this->exp = 100;
    this->cur_exp = 0;
    this->level = 1;
    this->kind = pokemonKind::highBlood;
    ...
    setBaseAttribute(static_cast<unsigned int>(BASEATTACK*(1+coForNormal[tempRarity])),
                     ...
                     static_cast<unsigned int>(MAXHP*(1+coForMain[tempRarity])),
                     double(WSP*(1+coForNormal[tempRarity])),
                     double(AVOID*(1+coForNormal[tempRarity])),
                     double(CRITICAL*(1+coForNormal[tempRarity]))); //血量与其他属性系数不同
}

QString HighBlood::getKind() {
...
void HighBlood::levelUp() {
    ...
    setBaseAttribute(static_cast<unsigned int>(base_attack*(1+coForNormal[tempRarity])*(1+coLevelupNormal)),
                     ...
                     static_cast<unsigned int>(max_hp*(1+coForMain[tempRarity])*(1+coLevelupMain)),
                     double (wsp*(1+coForNormal[tempRarity])*(1+coLevelupNormal)),
                     double (avoid*(1+coForNormal[tempRarity])*(1+coLevelupNormal)),
                     double (critical*(1+coForNormal[tempRarity])*(1+coLevelupNormal))); //血量的增长与其他属性不一样
}
