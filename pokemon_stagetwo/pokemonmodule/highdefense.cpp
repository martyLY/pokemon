...
//防御值相较于其他类型精灵更高
//攻击速度, 闪避率, 暴击率与其他精灵持平
static const unsigned int BASEATTACK = 190;
static const unsigned int DEFENSEPOWER = 130;
static const unsigned int MAXHP = 1000;
static const double WSP = 0.4;
static const double AVOID = 0.02;
...
//分为主要属性和次要属性
void HighDefense::initPokemon() {
    this->exp = 100;
    this->cur_exp = 0;
    this->level = 1;
    this->kind = pokemonKind::highDefense;
    ...
    setBaseAttribute(static_cast<unsigned int>(BASEATTACK*(1+coForNormal[tempRarity])),
                     static_cast<unsigned int>(DEFENSEPOWER*(1+coForMain[tempRarity])),
                     ...
                     double(CRITICAL*(1+coForNormal[tempRarity]))); //防御力与其他属性系数不同
}

QString HighDefense::getKind() {
...
void HighDefense::levelUp() {
    ...
    setBaseAttribute(static_cast<unsigned int>(base_attack*(1+coForNormal[tempRarity])*(1+coLevelupNormal)),
                     static_cast<unsigned int>(defense_power*(1+coForMain[tempRarity])*(1+coLevelupMain)),
                     ...
                     double (critical*(1+coForNormal[tempRarity])*(1+coLevelupNormal))); //防御力的增长与其他属性不一样
}
